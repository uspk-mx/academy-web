import { CreateQuizDocument } from "gql-generated/gql/graphql";
import type {
  CreateQuizInput,
  CreateQuizMutation,
...
export function CreateQuizDialog({
  ...
  const onCreateQuizMutation = async (formData: FormData) => {
    ...
    try {
      setIsSaving(true);
      const response = await createQuizMutation({
        input,
      });

      if (response.error) {
        setIsSaving(false);
        toast.error("Ocurrio un error al crear el quiz", {
      ...
      setTopicContent([
        ...topicContent,
        ...(response.data?.createQuiz ? [response.data.createQuiz] : []),
      ]);
    ...
    <Drawer onOpenChange={setOpen} open={open}>
      ...
      <DrawerContent className="w-11/12 gap-y-0 pt-0! pb-32 xl:pb-0 sm:px-0 sm:max-w-267.5 left-0 top-24!">
        ...
        >
            ...
            <div className="flex flex-col items-start xl:border-l w-full xl:border-l-border">
              ...
              <div className="flex flex-col items-start w-full px-6 pt-6 gap-6">
                ...
                <div className="space-y-2 w-full">
                  ...
                  <Input id="maxAttempts" name="maxAttempts" type="number" />
                </div>
              </div>
            </div>
          </div>
          <div
            className={cn(
              "hidden absolute inset-0 bg-muted-foreground/40 z-10",
              {
